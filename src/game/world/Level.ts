...
export class Level {
  ...
  private renderEdgeGateways(ctx: CanvasRenderingContext2D) {
    ...
    for (const trigger of this.edgeTriggers) {
      ...
      if (isRight) {
        ...
        ctx.fillStyle = penMid;
        ctx.fillText('[Borda da Tela]', archX + 60, groundY - 136);
      } else {
        ...
        ctx.font = 'italic 10px "Caveat", cursive';
        ctx.fillStyle = penMid;
        ctx.fillText('[Borda da Tela]', postX - 60, groundY - 136);
      }
  ...
  private renderLoreNote(ctx: CanvasRenderingContext2D, note: LoreNote) {
    ctx.save();
    ctx.font = 'italic bold 11px "Caveat", "Special Elite", cursive, sans-serif';
    ctx.fillStyle = GAME_CONFIG.PALETTE.PEN_PRIMARY;
    ctx.textAlign = 'left';

    // Pequeno alfinete ou cruz no topo do bilhete
    ctx.strokeStyle = GAME_CONFIG.PALETTE.MARGIN_LINE;
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.moveTo(note.x - 10, note.y - 12);
    ctx.lineTo(note.x + 180, note.y - 12);
    ctx.stroke();

    ctx.fillText(note.title, note.x, note.y - 2);

    ctx.font = '10px "Special Elite", "Courier New", monospace';
    ctx.fillStyle = GAME_CONFIG.PALETTE.PEN_DARKEST;
    let currY = note.y + 12;
    for (const line of note.lines) {
      ctx.fillText(line, note.x, currY);
      currY += 13;
    }
    ctx.restore();
  }
