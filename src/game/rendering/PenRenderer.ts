...
export class PenRenderer {
  ...
  public renderPlatform(
    ...
    // Cor interna sólida para ocultar o fundo
    ctx.fillStyle = GAME_CONFIG.PALETTE.PAPER_BG;
    ctx.fillRect(x, y, w, h);

    // Contorno principal em caneta esferográfica azul
    ctx.strokeStyle = GAME_CONFIG.PALETTE.PEN_PRIMARY;
    ctx.lineWidth = 2.4;
    ctx.lineJoin = 'miter';
    ctx.strokeRect(x, y, w, h);

    // Linha superior com grama / raízes / ranhuras manuais
    ctx.beginPath();
    ctx.strokeStyle = GAME_CONFIG.PALETTE.PEN_DARKEST;
    ctx.lineWidth = 2;
    for (let gx = x + 4; gx < x + w - 4; gx += 8) {
      const bladeH = 4 + ((gx * 13) % 7);
      ctx.moveTo(gx, y);
      ctx.lineTo(gx - 2, y - bladeH);
    }
    ctx.stroke();

    // Hachura densa de terra/pedra (cross-hatching)
    ctx.beginPath();
    ctx.strokeStyle = GAME_CONFIG.PALETTE.PEN_HATCHING;
    ctx.lineWidth = 1;
    const hatchStep = 12;
    ...
    // Raízes pendentes abaixo de plataformas flutuantes
    if (type === 'FLOATING') {
      ctx.beginPath();
      ctx.strokeStyle = GAME_CONFIG.PALETTE.PEN_PRIMARY;
      ctx.lineWidth = 1.6;
      for (let rx = x + 15; rx < x + w - 15; rx += 25) {
        const rootLen = 14 + ((rx * 7) % 18);
        ctx.moveTo(rx, y + h);
        ctx.quadraticCurveTo(rx + 4, y + h + rootLen * 0.6, rx - 3, y + h + rootLen);
      }
      ctx.stroke();
    }
