...
export class ParallaxBackgroundSystem {
  ...
  private renderMonasteryCloudsToBuffer(canvas: HTMLCanvasElement) {
    ...
    // 1. NUVENS SUTIS E ESTRATIFICADAS (Alto do céu, traços horizontais finos de caneta)
    ctx.strokeStyle = 'rgba(20, 61, 153, 0.3)';
    ctx.lineWidth = 0.9;
    const stratusLines = [
      { x: 120, y: 35, len: 160 }, { x: 340, y: 55, len: 240 },
      { x: 780, y: 40, len: 310 }, { x: 1220, y: 30, len: 260 },
      { x: 1580, y: 60, len: 220 }, { x: 90, y: 80, len: 180 },
      { x: 1040, y: 75, len: 200 }
    ];
    ctx.beginPath();
    for (const st of stratusLines) {
      ctx.moveTo(st.x, st.y);
      ctx.lineTo(st.x + st.len, st.y);
      ctx.moveTo(st.x + 20, st.y + 4);
      ctx.lineTo(st.x + st.len - 15, st.y + 4);
    }
    ctx.stroke();

    // 2. NUVEM DE CHUVA / ACÚMULO PRINCIPAL (Inspirada no imponente cúmulo central da prancha)
    const cumulusCenters = [
      { x: 740, y: 140, scale: 1.15 },
      { x: 1640, y: 155, scale: 0.85 },
      { x: 200, y: 165, scale: 0.75 }
    ];

    for (const c of cumulusCenters) {
      this.drawStormCumulus(ctx, c.x, c.y, c.scale, penMid, penHatching);
    }

    ctx.restore();
  }

  // Desenha uma nuvem de acúmulo tempestuosa idêntica ao esboço da prancha 04
  private drawStormCumulus(
    ...
    // Contorno bulboso com arcos de caneta sobrepostos
    ctx.fillStyle = 'rgba(246, 240, 224, 0.95)';
    ctx.strokeStyle = penColor;
    ctx.lineWidth = 1.6;

    ctx.beginPath();
    // Base plana com ranhuras de chuva
    ctx.moveTo(-180, 50);
    ctx.lineTo(190, 50);
    // Cúpulas superiores arredondadas
    ctx.arc(170, 30, 28, 0.1, -Math.PI * 0.4, true);
    ctx.arc(130, -5, 36, 0.2, -Math.PI * 0.6, true);
    ctx.arc(80, -45, 48, 0.1, -Math.PI * 0.7, true);
    ctx.arc(0, -75, 62, 0.2, -Math.PI * 0.8, true); // Topo imponente central
    ctx.arc(-85, -50, 48, -0.1, -Math.PI * 0.9, true);
    ctx.arc(-145, -15, 38, -0.1, -Math.PI * 0.9, true);
    ctx.arc(-175, 25, 28, -0.2, -Math.PI * 0.9, true);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();

    // Linhas internas de volume (volutas concêntricas à caneta)
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.arc(0, -35, 42, Math.PI * 0.8, Math.PI * 1.8);
    ctx.arc(70, -10, 32, Math.PI * 0.7, Math.PI * 1.7);
    ctx.arc(-65, -15, 30, Math.PI * 0.9, Math.PI * 1.9);
    ctx.stroke();

    // HACHURA PESADA DE CHUVA / SOMBRA NA BASE (exatamente como desenhado na prancha)
    ctx.strokeStyle = hatchColor;
    ctx.lineWidth = 1.0;
    ctx.beginPath();
    for (let hx = -170; hx <= 180; hx += 8) {
      ctx.moveTo(hx, 15);
      ctx.lineTo(hx - 22, 54);
    }
    // Hachura cruzada no centro mais denso
    for (let hx = -110; hx <= 120; hx += 10) {
      ctx.moveTo(hx - 15, 20);
      ctx.lineTo(hx + 15, 52);
    }
    ctx.stroke();
  ...
  private renderMonasteryBiome(ctx: CanvasRenderingContext2D, cameraX: number, cameraY: number) {
    ...
    // CAMADA 3: Silhueta de Castelos, Bastiões, Andaimes e Arcos de Pedra (Background 03)
    // Parallax intermediário (0.28x)
    if (this.monasteryCastlesBuffer) {
      const castleFactor = 0.28;
      const castleOffset = ((cameraX * castleFactor) % this.loopWidth + this.loopWidth) % this.loopWidth;
      ctx.drawImage(this.monasteryCastlesBuffer, -castleOffset, 0);
      ctx.drawImage(this.monasteryCastlesBuffer, -castleOffset + this.loopWidth, 0);
      if (-castleOffset + this.loopWidth < w) {
        ctx.drawImage(this.monasteryCastlesBuffer, -castleOffset + this.loopWidth * 2, 0);
      }
    }
